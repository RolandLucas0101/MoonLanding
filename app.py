...
def display_moving_simulation(data):
    ...
    try:
        # Force reload the visualization module to avoid caching issues
        import importlib
        import visualization
        importlib.reload(visualization)
        
        visualizer = visualization.TrajectoryVisualizer()
        
        # Create the animated plot
        fig = create_animated_plot(data, current_step)
        
        # Display the plot with unique key
        st.plotly_chart(fig, use_container_width=True, key=f"animated_plot_{current_step}")
        
    except Exception as e:
        st.error(f"Error creating animation: {str(e)}")
        st.write("Please try refreshing the page or recalculating the trajectory.")
...
def create_animated_plot(trajectory_data, current_step):
    """Create animated trajectory plot with rocket position"""
    ...
    # Add full trajectory path (faded)
    ...
    # Add trajectory up to current position
    if current_step > 0:
        fig.add_trace(go.Scatter(
            x=positions[:current_step+1, 0],
            y=positions[:current_step+1, 1],
            mode='lines',
            name='Completed Path',
    ...
    earth_x = earth_radius * np.cos(earth_theta)
    earth_y = earth_radius * np.sin(earth_theta)
    fig.add_trace(go.Scatter(
        x=earth_x, y=earth_y,
        mode='lines',
        fill='toself',
    ...
    # Add Moon at current position
    if current_step < len(times):
        # Simple moon position (circular orbit)
        moon_distance = 384400  # km
        moon_orbital_period = 27.3 * 24 * 3600  # seconds
        moon_angle = (times[current_step] / moon_orbital_period) * 2 * np.pi
        moon_x = moon_distance * np.cos(moon_angle)
        moon_y = moon_distance * np.sin(moon_angle)
        
        moon_theta = np.linspace(0, 2*np.pi, 50)
        moon_xs = moon_x + moon_radius * np.cos(moon_theta)
        moon_ys = moon_y + moon_radius * np.sin(moon_theta)
        fig.add_trace(go.Scatter(
            x=moon_xs, y=moon_ys,
            mode='lines',
            fill='toself',
    ...
    # Add rocket at current position
    if current_step < len(positions):
        rocket_pos = positions[current_step]
        
        # Create rocket shape (triangle pointing in direction of motion)
        velocity = trajectory_data['velocities'][current_step] if current_step < len(trajectory_data['velocities']) else [0, 0]
        rocket_size = 2000  # km
        
        # Calculate rocket orientation based on velocity
        if np.linalg.norm(velocity) > 0:
            vel_angle = np.arctan2(velocity[1], velocity[0])
        else:
            vel_angle = 0
        
        # Create rocket triangle
        rocket_angles = np.array([0, 2*np.pi/3, 4*np.pi/3, 0]) + vel_angle
        rocket_x = rocket_pos[0] + rocket_size * np.cos(rocket_angles)
        rocket_y = rocket_pos[1] + rocket_size * np.sin(rocket_angles)
        ...
        fig.add_trace(go.Scatter(
            ...
            name='🚀 Spacecraft',
            marker=dict(size=8),
            hovertemplate=f'Spacecraft Position<br>X: {rocket_pos[0]:.0f} km<br>Y: {rocket_pos[1]:.0f} km<br>Time: {times[current_step]/3600:.1f} hours<extra></extra>'
        ))
        
        # Add velocity vector
        vel_scale = 10000  # Scale factor for visibility
        vel_end_x = rocket_pos[0] + velocity[0] * vel_scale
        vel_end_y = rocket_pos[1] + velocity[1] * vel_scale
        
        fig.add_trace(go.Scatter(
            x=[rocket_pos[0], vel_end_x],
            y=[rocket_pos[1], vel_end_y],
            mode='lines+markers',
            line=dict(color='orange', width=3),
            marker=dict(size=[0, 8], symbol=['circle', 'triangle-up']),
            name='Velocity Vector',
            hovertemplate=f'Velocity: {np.linalg.norm(velocity):.3f} km/s<extra></extra>'
        ))
    
    # Update layout
    fig.update_layout(
        title=f'Moon Launch Animation - Step {current_step + 1}/{len(positions)}',
    ...
    # Set axis ranges to show relevant area
    if current_step < len(positions):
        current_pos = positions[current_step]
        margin = 50000  # km
        
        fig.update_xaxes(range=[current_pos[0] - margin, current_pos[0] + margin])
        fig.update_yaxes(range=[current_pos[1] - margin, current_pos[1] + margin])
