...
def display_moving_simulation(data):
    ...
    try:
        ...
        st.write(f"Debug: Method exists? {hasattr(visualizer, 'plot_animated_trajectory')}")
        st.write(f"Debug: Available methods: {[m for m in dir(visualizer) if not m.startswith('_')]}")
        
        # Create a simple version first
        if hasattr(visualizer, 'plot_animated_trajectory'):
            fig = visualizer.plot_animated_trajectory(data, current_step)
        else:
            # Fallback to regular trajectory plot
            st.warning("Using fallback trajectory plot")
            fig = visualizer.plot_trajectory(data)
